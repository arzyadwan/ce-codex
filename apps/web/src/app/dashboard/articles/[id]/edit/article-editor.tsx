...
type Article = { id: string; title: string; slug: string; excerpt: string; seoTitle: string | null; seoDescription: string | null; status: string; contentType: "news" | "analysis" | "opinion" | "education" | "press_release" | "sponsored"; sources: Array<{ title: string; url: string; publisher?: string }>; sponsorName: string | null; sponsorUrl: string | null; sponsorDisclosure: string | null; affiliateDisclosure: string | null; content: RichTextNode; category: { slug: string; name: string } | null; tags: Array<{ name: string; slug: string }>; canEdit: boolean; revisionNotes: Array<{ id: string; note: string; requestedBy: string; createdAt: string; resolvedAt: string | null }> };
...
export function ArticleEditor({ articleId }: { articleId: string }) {
  ...
  async function persist(formElement: HTMLFormElement, automatic: boolean) {
    ...
    try {
      ...
      const sources = String(form.get("sources") ?? "").split("\n").map((line) => line.trim()).filter(Boolean).map((line) => { const [title, url, publisher] = line.split("|").map((part) => part.trim()); return { title, url, ...(publisher ? { publisher } : {}) }; });
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/articles/${articleId}`, { method: "PATCH", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ title: form.get("title"), slug: form.get("slug"), excerpt: form.get("excerpt"), categorySlug: form.get("categorySlug") || undefined, tags: String(form.get("tags") ?? "").split(",").map((tag) => tag.trim()).filter(Boolean), contentType: form.get("contentType"), sources, sponsorName: form.get("sponsorName") || undefined, sponsorUrl: form.get("sponsorUrl") || undefined, sponsorDisclosure: form.get("sponsorDisclosure") || undefined, affiliateDisclosure: form.get("affiliateDisclosure") || undefined, seoTitle: form.get("seoTitle") || undefined, seoDescription: form.get("seoDescription") || undefined, content: editor.getJSON() }) });
      ...
      setArticle((current) => current ? { ...current, ...updated, canEdit: current.canEdit, revisionNotes: current.revisionNotes } : current);
  ...
  return <form ref={formRef} className="article-editor-shell" onSubmit={save}>
    <section className="article-editor-main">
      ...
      {preview ? <article className="editor-preview"><div className="card-labels"><p className="kicker">PRATINJAU</p>{article.contentType === "sponsored" ? <span className="commercial-label">Konten sponsor</span> : null}</div><h1>{article.title}</h1><p className="article-deck">{article.excerpt}</p>{article.sponsorDisclosure ? <aside className="commercial-disclosure"><strong>Transparansi konten:</strong> {article.sponsorDisclosure}</aside> : null}<div className="article-body"><RichTextContent document={editor?.getJSON() as RichTextNode} /></div></article> : <>
        ...
        <fieldset className="editor-metadata-fields" disabled={readOnly}><legend>Klasifikasi & transparansi</legend><label htmlFor="edit-content-type">Jenis konten</label><select id="edit-content-type" name="contentType" defaultValue={article.contentType} onChange={scheduleAutosave}><option value="news">Berita</option><option value="analysis">Analisis</option><option value="opinion">Opini</option><option value="education">Edukasi</option><option value="press_release">Siaran pers</option><option value="sponsored">Konten sponsor</option></select><label htmlFor="edit-sources">Sumber dan referensi</label><textarea id="edit-sources" name="sources" rows={4} defaultValue={article.sources.map((source) => `${source.title} | ${source.url}${source.publisher ? ` | ${source.publisher}` : ""}`).join("\n")} aria-describedby="edit-sources-help" onChange={scheduleAutosave} /><small id="edit-sources-help">Satu sumber per baris: judul | URL | penerbit.</small><div className="commercial-editor-grid"><div><label htmlFor="edit-sponsor-name">Nama sponsor</label><input id="edit-sponsor-name" name="sponsorName" defaultValue={article.sponsorName ?? ""} onChange={scheduleAutosave} /></div><div><label htmlFor="edit-sponsor-url">URL sponsor</label><input id="edit-sponsor-url" name="sponsorUrl" type="url" defaultValue={article.sponsorUrl ?? ""} onChange={scheduleAutosave} /></div></div><label htmlFor="edit-sponsor-disclosure">Disclosure sponsor</label><textarea id="edit-sponsor-disclosure" name="sponsorDisclosure" rows={3} minLength={20} maxLength={1000} defaultValue={article.sponsorDisclosure ?? ""} onChange={scheduleAutosave} /><label htmlFor="edit-affiliate-disclosure">Disclosure afiliasi</label><textarea id="edit-affiliate-disclosure" name="affiliateDisclosure" rows={3} minLength={20} maxLength={1000} defaultValue={article.affiliateDisclosure ?? ""} onChange={scheduleAutosave} /></fieldset>
