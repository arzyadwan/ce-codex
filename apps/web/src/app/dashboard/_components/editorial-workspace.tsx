...
export function EditorialWorkspace({ email }: { email: string }) {
  ...
  async function createDraft(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const payload = { title, slug: String(form.get("slug")), excerpt: String(form.get("excerpt")), featuredImageUrl, categorySlug: form.get("categorySlug") || undefined, tags: String(form.get("tags") ?? "").split(",").map((tag) => tag.trim()).filter(Boolean), content: { type: "doc", content: [{ type: "paragraph", content: [{ type: "text", text: String(form.get("content")) }] }] } };
